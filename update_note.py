    ...
    while True:
        ...
        if update_field in note.keys() and update_field != 'created_date':
            ...
            if update_field == 'issue_date':
                note.update({update_field : check_date(note.get('issue_date'))})
            # Изменение статуса:
            elif update_field == 'status':
                note.update({update_field : check_status(note.get('status'))})
